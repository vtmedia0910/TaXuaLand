  ...
  return transaction(async (client) => {
    const release = (
      await client.query<{
        ...
        `SELECT r.id,r.dataset_id,r.qa_status,s.public_display,s.redistribution,s.derivatives,(d.kind IN ('TERRAIN','ROADS','IMAGERY') AND s.status='ACTIVE' AND s.archived_at IS NULL AND s.caching='ALLOWED' AND s.license_reference IS NOT NULL AND r.bbox IS NOT NULL AND ((d.kind IN ('TERRAIN','ROADS') AND r.target_crs='EPSG:4326') OR (d.kind='IMAGERY' AND r.source_crs='EPSG:32648' AND r.target_crs='EPSG:3857' AND r.vertical_datum='UNKNOWN' AND r.resolution=10 AND EXISTS(SELECT 1 FROM areas_of_interest aoi WHERE aoi.active AND ST_Covers(r.bbox,aoi.boundary)))) AND (d.kind<>'TERRAIN' OR (r.vertical_datum='WGS84_ELLIPSOID' AND r.resolution IS NOT NULL)) AND EXISTS(SELECT 1 FROM dataset_assets a WHERE a.release_id=r.id AND a.zone='published' AND a.public_url IS NOT NULL AND a.checksum=r.checksum AND a.content_type=CASE d.kind WHEN 'TERRAIN' THEN 'application/vnd.land.terrain+json' WHEN 'IMAGERY' THEN 'application/vnd.land.imagery+json' ELSE 'application/geo+json' END) AND EXISTS(SELECT 1 FROM pipeline_runs p WHERE p.release_id=r.id AND p.processing_version=r.pipeline_version AND p.status='COMPLETED' AND NOT EXISTS(SELECT 1 FROM pipeline_runs duplicate WHERE duplicate.release_id=p.release_id AND duplicate.processing_version=p.processing_version AND duplicate.status='COMPLETED' AND duplicate.id<>p.id) AND (d.kind<>'IMAGERY' OR (p.manifest @> '{"registration":"APPROVED_OPERATOR","verificationStatus":"UNKNOWN","accuracy":"UNKNOWN"}'::jsonb AND p.manifest->>'sourceLockSha256'=r.source_version AND p.manifest->>'manifestSha256'=r.checksum AND p.manifest#>'{quality,aoiContained}'='true'::jsonb AND p.manifest#>'{quality,sceneCloudPercent}'>='0'::jsonb AND p.manifest#>'{quality,sceneCloudPercent}'<='20'::jsonb AND p.manifest#>'{quality,obstructionThresholdPercent}'='10'::jsonb AND p.manifest#>'{quality,aoiObstructionPercent}'>='0'::jsonb AND p.manifest#>'{quality,aoiObstructionPercent}'<='10'::jsonb AND p.manifest#>'{quality,nodataPixelCount}'='0'::jsonb AND p.manifest#>'{quality,saturatedPixelCount}'='0'::jsonb AND p.manifest#>'{quality,obstructionClasses}'='[3,8,9,10,11]'::jsonb)))) AS ready FROM dataset_releases r JOIN datasets d ON d.id=r.dataset_id JOIN sources s ON s.id=d.source_id WHERE r.id=$1 FOR UPDATE OF r`,
        [id],
      )
    ).rows[0];
    if (
